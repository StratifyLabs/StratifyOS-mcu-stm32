...
static DMA_Stream_TypeDef *const stm32_dma0_regs[MCU_DMA_STREAM_COUNT]
  = MCU_DMA0_REGS;
static DMA_Stream_TypeDef *const stm32_dma1_regs[MCU_DMA_STREAM_COUNT]
  = MCU_DMA1_REGS;
...
stm32_dma_channel_t *stm32_dma_set_handle(u32 dma_number, u32 stream_number) {
  if ((dma_number < MCU_DMA_PORTS) && (stream_number < MCU_DMA_STREAM_COUNT)) {
    ...
    interrupt_number
      = stm32_dma_get_interrupt_number(dma_number, stream_number);
...
void stm32_dma_clear_handle(u32 dma_number, u32 stream_number) {
  ...
  if ((dma_number < MCU_DMA_PORTS) && (stream_number < MCU_DMA_STREAM_COUNT)) {
    ...
    stm32_dma_channel_t *channel
      = &stm32_dma_handle[dma_number].stream[stream_number];
    // ON STM32 Interrupt zero is the WDT (or something else that typically
    // doesn't support DMA -- so zero is not valid
    if (channel->interrupt_number <= 0) {
      return;
    }
...
stm32_dma_setattr(const stm32_dma_channel_config_t *config) {
  ...
  channel->handle.Instance
    = stm32_dma_get_stream_instance(config->dma_number, config->stream_number);
  ...
  channel->handle.Init.Direction
    = DMA_PERIPH_TO_MEMORY; // read is always periph to memory
  if (o_flags & STM32_DMA_FLAG_IS_MEMORY_TO_PERIPH) {
    channel->handle.Init.Direction
      = DMA_MEMORY_TO_PERIPH; // read is always periph to memory
  }
  ...
  if (o_flags & STM32_DMA_FLAG_IS_MEMORY_INC_DISABLE) {
    channel->handle.Init.MemInc = DMA_MINC_DISABLE;
  }

  if (o_flags & STM32_DMA_FLAG_IS_PERIPH_INC_ENABLE) {
    channel->handle.Init.PeriphInc = DMA_PINC_ENABLE;
  }
  ...
  if (o_flags & STM32_DMA_FLAG_IS_FIFO) {
    ...
    if (o_flags & STM32_DMA_FLAG_IS_FIFO_THRESHOLD_QUARTER) {
      channel->handle.Init.FIFOThreshold = DMA_FIFO_THRESHOLD_1QUARTERFULL;
    }

    if (o_flags & STM32_DMA_FLAG_IS_FIFO_THRESHOLD_THREE_QUARTER) {
      channel->handle.Init.FIFOThreshold = DMA_FIFO_THRESHOLD_3QUARTERSFULL;
    }

    if (o_flags & STM32_DMA_FLAG_IS_FIFO_THRESHOLD_FULL) {
      channel->handle.Init.FIFOThreshold = DMA_FIFO_THRESHOLD_FULL;
    }

    channel->handle.Init.MemBurst = DMA_MBURST_SINGLE;
    if (o_flags & STM32_DMA_FLAG_IS_MEMORY_INC4) {
      channel->handle.Init.MemBurst = DMA_MBURST_INC4;
    }

    if (o_flags & STM32_DMA_FLAG_IS_MEMORY_INC8) {
      channel->handle.Init.MemBurst = DMA_MBURST_INC8;
    }

    if (o_flags & STM32_DMA_FLAG_IS_MEMORY_INC16) {
      channel->handle.Init.MemBurst = DMA_MBURST_INC16;
    }

    channel->handle.Init.PeriphBurst = DMA_PBURST_SINGLE;
    if (o_flags & STM32_DMA_FLAG_IS_PERIPH_INC4) {
      channel->handle.Init.PeriphBurst = DMA_PBURST_INC4;
    }

    if (o_flags & STM32_DMA_FLAG_IS_PERIPH_INC8) {
      channel->handle.Init.PeriphBurst = DMA_PBURST_INC8;
    }

    if (o_flags & STM32_DMA_FLAG_IS_PERIPH_INC16) {
      channel->handle.Init.PeriphBurst = DMA_PBURST_INC16;
    }
...
static void mcu_core_dma_handler(int dma_number, int stream_number) {
  HAL_DMA_IRQHandler(
    &stm32_dma_handle[dma_number].stream[stream_number].handle);
}
...
MDMA_HandleTypeDef stm32_dma_mdma_handle;

void mcu_core_mdma_isr() { HAL_MDMA_IRQHandler(&stm32_dma_mdma_handle); }
#endif
