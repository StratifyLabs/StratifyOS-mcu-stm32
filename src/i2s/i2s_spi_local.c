...
int i2s_spi_local_setattr(const devfs_handle_t *handle, void *ctl) {
  ...
  if (o_flags & (I2S_FLAG_SET_MASTER | I2S_FLAG_SET_SLAVE)) {
    ...
    switch (attr->freq) {
    case 11000:
      local->i2s_hal_handle.Init.AudioFreq = I2S_AUDIOFREQ_11K;
      break;
    case 16000:
      local->i2s_hal_handle.Init.AudioFreq = I2S_AUDIOFREQ_16K;
      break;
    case 22050:
      local->i2s_hal_handle.Init.AudioFreq = I2S_AUDIOFREQ_22K;
      break;
    case 32000:
      local->i2s_hal_handle.Init.AudioFreq = I2S_AUDIOFREQ_32K;
      break;
    case 44100:
      local->i2s_hal_handle.Init.AudioFreq = I2S_AUDIOFREQ_44K;
      break;
    case 48000:
      local->i2s_hal_handle.Init.AudioFreq = I2S_AUDIOFREQ_48K;
      break;
    case 96000:
      local->i2s_hal_handle.Init.AudioFreq = I2S_AUDIOFREQ_96K;
      break;
    case 192000:
      local->i2s_hal_handle.Init.AudioFreq = I2S_AUDIOFREQ_192K;
      break;
    default:
      return SYSFS_SET_RETURN(EINVAL);
    }
    ...
    local->i2s_hal_handle.Init.Data24BitAlignment
      = I2S_DATA_24BIT_ALIGNMENT_RIGHT;
#endif

#if defined I2S_MASTER_KEEP_IO_STATE_DISABLE
    local->i2s_hal_handle.Init.MasterKeepIOState
...
void i2s_spi_local_wait_for_errata_level(spi_local_t *local) {
  ...
  if (local->o_flags & SPI_LOCAL_IS_ERRATA_REQUIRED) {
    ...
    target_level
      = (local->o_flags & SPI_LOCAL_IS_ERRATA_I2S) == 0; // MSB : 1, I2S: 0
    do {
...
void HAL_I2S_RxCpltCallback(I2S_HandleTypeDef *hi2s) {
  ...
  if (local->o_flags & SPI_LOCAL_IS_DMA && async) {
    mcu_core_invalidate_data_cache_block(
      ((u8 *)local->transfer_handler.read->buf)
        + local->transfer_handler.read->nbyte / 2,
      local->transfer_handler.read->nbyte / 2);
  }
  result = devfs_execute_read_handler(
