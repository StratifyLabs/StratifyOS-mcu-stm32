...
#if MCU_TMR_API == 1
	if( flags & MR0_FLAG ){
		mcu_execute_event_handler(&(_mcu_tmr_local[port].handler[0]), (mcu_event_t)regs->MR[0]);
	}
	if( flags & MR1_FLAG ){
		mcu_execute_event_handler(&(_mcu_tmr_local[port].handler[1]), (mcu_event_t)regs->MR[1]);
	}
	if( flags & MR2_FLAG ){
		mcu_execute_event_handler(&(_mcu_tmr_local[port].handler[2]), (mcu_event_t)regs->MR[2]);
	}
	if( flags & MR3_FLAG ){
		mcu_execute_event_handler(&(_mcu_tmr_local[port].handler[3]), (mcu_event_t)regs->MR[3]);
	}
	if( flags & CR0_FLAG ){
		mcu_execute_event_handler(&(_mcu_tmr_local[port].handler[4]), (mcu_event_t)regs->CR[0]);
	}
	if( flags & CR1_FLAG ){
		mcu_execute_event_handler(&(_mcu_tmr_local[port].handler[5]), (mcu_event_t)regs->CR[1]);
	}
#else
	if( flags & MR0_FLAG ){
		mcu_execute_event_handler(&(_mcu_tmr_local[port].handler[0]), (mcu_event_t)regs->MR0);
	}
	if( flags & MR1_FLAG ){
		mcu_execute_event_handler(&(_mcu_tmr_local[port].handler[1]), (mcu_event_t)regs->MR1);
	}
	if( flags & MR2_FLAG ){
		mcu_execute_event_handler(&(_mcu_tmr_local[port].handler[2]), (mcu_event_t)regs->MR2);
	}
	if( flags & MR3_FLAG ){
		mcu_execute_event_handler(&(_mcu_tmr_local[port].handler[3]), (mcu_event_t)regs->MR3);
	}
	if( flags & CR0_FLAG ){
		mcu_execute_event_handler(&(_mcu_tmr_local[port].handler[4]), (mcu_event_t)regs->CR0);
	}
	if( flags & CR1_FLAG ){
		mcu_execute_event_handler(&(_mcu_tmr_local[port].handler[5]), (mcu_event_t)regs->CR1);
	}
#endif
}
