...
int mcu_pio_setaction(const devfs_handle_t *handle, void *ctl) {
  ...
  if (action->channel < 16) {
    ...
    cortexm_set_irq_priority(
      pio_local_event_irq_numbers[action->channel],
      action->prio,
      action->o_events);
  } else {
    return SYSFS_SET_RETURN(EINVAL);
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin) {
  ...
  pio_local_event_handler_t *local_handler
    = pio_local_event_handler + pin_number;
  pio_event_data_t event_data;
  event_data.status = local_handler->pio_regs->IDR;
  event_data.falling = ((local_handler->o_events & MCU_EVENT_FLAG_FALLING) != 0)
                       << pin_number;
  event_data.rising = ((local_handler->o_events & MCU_EVENT_FLAG_RISING) != 0)
                      << pin_number;

  if (
    devfs_execute_event_handler(
      &local_handler->handler,
      local_handler->o_events,
      &event_data)
    == 0) {
...
void mcu_core_exti0_isr() { HAL_GPIO_EXTI_IRQHandler(1 << 0); }

void mcu_core_exti1_isr() { HAL_GPIO_EXTI_IRQHandler(1 << 1); }

void mcu_core_exti2_isr() { HAL_GPIO_EXTI_IRQHandler(1 << 2); }

void mcu_core_exti3_isr() { HAL_GPIO_EXTI_IRQHandler(1 << 3); }

void mcu_core_exti4_isr() { HAL_GPIO_EXTI_IRQHandler(1 << 4); }
