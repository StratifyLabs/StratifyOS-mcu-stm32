...
int mcu_qspi_setattr(const devfs_handle_t * handle, void * ctl){
	...
	if(o_flags & QSPI_FLAG_READ_REGISTER){
		...
		command.Instruction       = attr->command.opcode;
		command.AddressMode       = QSPI_ADDRESS_NONE;
		command.NbData = attr->command.data_size;
		...
		if (HAL_QSPI_Receive(&qspi->hal_handle, attr->command.data,HAL_QPSI_TIMEOUT_DEFAULT_VALUE) != HAL_OK){
		...
		if (command.NbData){
			if(HAL_QSPI_Transmit(&qspi->hal_handle, attr->command.data, HAL_QPSI_TIMEOUT_DEFAULT_VALUE)!=HAL_OK){
				return SYSFS_SET_RETURN(EIO);
			}
		}
		return 0;
