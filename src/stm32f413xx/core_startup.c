// Copyright 2011-2022 Tyler Gilbert and Stratify Labs, Inc; see LICENSE.md
...
_DECLARE_ISR(dfsdm1_flt0);        // 61
_DECLARE_ISR(dfsdm1_flt1);        // 62
...
_DECLARE_ISR(can3_tx);            // 74
_DECLARE_ISR(can3_rx0);           // 75
_DECLARE_ISR(can3_rx1);           // 76
_DECLARE_ISR(can3_sce);           // 77
_DECLARE_ISR(rng);                // 80
_DECLARE_ISR(fpu);                // 81
...
_DECLARE_ISR(uart9);              // 88
_DECLARE_ISR(uart10);             // 89
_DECLARE_ISR(quadspi);            // 92
_DECLARE_ISR(fmpi2c1_ev);         // 95
_DECLARE_ISR(fmpi2c1_er);         // 96
_DECLARE_ISR(lptim1);             // 97
_DECLARE_ISR(dfsdm2_flt0);        // 98
_DECLARE_ISR(dfsdm2_flt1);        // 99
_DECLARE_ISR(dfsdm2_flt2);        // 100
_DECLARE_ISR(dfsdm2_flt3);        // 101
...
void (*const mcu_core_vector_table[])() __attribute__((section(".startup"))) = {
        ...
        _ISR(dfsdm2_flt0), // 98
        _ISR(dfsdm2_flt1), // 99
        _ISR(dfsdm2_flt2), // 100
        _ISR(dfsdm2_flt3), // 101
};
