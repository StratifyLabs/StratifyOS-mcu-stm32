...
DEVFS_MCU_DRIVER_IOCTL_FUNCTION_MIN(
  emc_fmc_ahb,
  EMC_VERSION,
  EMC_IOC_IDENT_CHAR)
...
static int mcu_emc_ram_getinfo(const devfs_handle_t *handle, void *ctl) {
  ...
  info->o_flags
    = EMC_FLAG_DISABLE | EMC_FLAG_ENABLE | EMC_FLAG_IS_SDRAM | EMC_FLAG_IS_PSRAM
      | EMC_FLAG_IS_SRAM | EMC_FLAG_IS_NOR | EMC_FLAG_IS_NAND
      | EMC_FLAG_IS_8B_ACCESS | EMC_FLAG_IS_16B_ACCESS | EMC_FLAG_IS_32B_ACCESS
      | EMC_FLAG_IS_AHB | EMC_FLAG_IS_PSRAM_BANK1 | EMC_FLAG_IS_PSRAM_BANK2
      | EMC_FLAG_IS_PSRAM_BANK3 | EMC_FLAG_IS_PSRAM_BANK4;
...
static int mcu_emc_ram_setattr(const devfs_handle_t *handle, void *ctl) {
  ...
  if (o_flags & EMC_FLAG_ENABLE) {
    ...
    if (
      HAL_SRAM_Init(&local->hal_handle, &local->timing, &local->timing)
      != HAL_OK) {
...
int mcu_emc_psram_read(const devfs_handle_t *handle, devfs_async_t *async) {
  ...
  if (async->loc & 0x03 || async->nbyte & 0x03) {
    HAL_SRAM_Read_8b(
      &(psram->hal_handle),
      (u32 *)(config->base_address + async->loc),
      async->buf,
      async->nbyte);
  } else {
    HAL_SRAM_Read_32b(
