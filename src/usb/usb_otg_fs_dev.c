...
void clear_callbacks(int port) {
  memset(
    m_usb_local[port].transfer_handler,
    0,
  ...
  memset(
    &m_usb_local[port].special_event_handler,
    0,
    sizeof(mcu_event_handler_t));
  memset(
    m_usb_local[port].rx_buffer_offset,
    0,
    MCU_USB_ENDPOINT_COUNT * sizeof(u16));
...
int mcu_usb_setattr(const devfs_handle_t *handle, void *ctl) {
  ...
  if (o_flags & USB_FLAG_SET_DEVICE) {
    ...
    HAL_PCDEx_SetRxFiFo(
      ...
      attr->rx_fifo_word_size); // size is in 32-bit words for all fifo - 512
...
void usb_configure_endpoint(
  ...
  HAL_PCD_EP_Open(
    &m_usb_local[handle->port].hal_handle,
    endpoint_num,
    max_packet_size,
    type & EP_TYPE_MSK);
  // m_usb_local[handle->port].connected = 1;
  ...
  if ((endpoint_num & 0x80) == 0) {
    ...
    if (m_usb_local[port].rx_buffer_offset[endpoint_num] == 0) {
      m_usb_local[port].rx_buffer_offset[endpoint_num]
        = m_usb_local[port].rx_buffer_used;
...
int mcu_usb_root_read_endpoint(
  ...
  if (m_usb_local[port].read_ready & (1 << epnum)) {
    ...
    src_buffer
      = stm32_config.usb_rx_buffer + m_usb_local[port].rx_buffer_offset[epnum];
...
int mcu_usb_root_write_endpoint(
  ...
  const int port = ((const usb_config_t *)(handle->config))->port;
#if MCU_USB_API > 0
...
void HAL_PCD_SetupStageCallback(PCD_HandleTypeDef *hpcd) {
  ...
  if (
    ...
    HAL_PCD_EP_Receive(
      hpcd,
      0,
      stm32_config.usb_rx_buffer + hpcd->OUT_ep[0].maxpacket,
      setup->wLength);
  }
}
...
void HAL_PCD_ResetCallback(PCD_HandleTypeDef *hpcd) {
  ...
  const u32 mps = stm32_config.usb_max_packet_zero;
  usb->connected = 1;
