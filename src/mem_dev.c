...
int mcu_mem_erasepage(const devfs_handle_t *handle, void *ctl) {
  ...
  page = (u32)ctl;
  addr
    = stm32_flash_get_sector_addr(page); // this gets the beginning of the page
...
int mcu_mem_writepage(const devfs_handle_t *handle, void *ctl) {
  ...
  if (wattr->addr + nbyte > (MCU_FLASH_SIZE + MCU_FLASH_START)) {
    nbyte
      = MCU_FLASH_SIZE
        - wattr
            ->addr; // update the bytes read to not go past the end of the disk
  }
...
int mcu_mem_read(const devfs_handle_t *cfg, devfs_async_t *rop) {
  MCU_UNUSED_ARGUMENT(cfg);
  if (
    (stm32_flash_is_flash(rop->loc, rop->nbyte))
...
int get_ram_page_size(int page) { return DEVICE_RAM_PAGE_SIZE; }
