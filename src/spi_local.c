...
int spi_local_setattr(const devfs_handle_t *handle, void *ctl) {
  ...
  if (o_flags & (SPI_FLAG_SET_SLAVE | SPI_FLAG_SET_MASTER)) {
    ...
    local->hal_handle.Init.MasterInterDataIdleness
      = SPI_MASTER_INTERDATA_IDLENESS_01CYCLE;
    local->hal_handle.Init.MasterReceiverAutoSusp
...
#if MCU_SPI_PORTS > 4
void mcu_core_spi5_isr() { HAL_SPI_IRQHandler(&m_spi_local[4].hal_handle); }
#endif

#if MCU_SPI_PORTS > 5
void mcu_core_spi6_isr() { HAL_SPI_IRQHandler(&m_spi_local[5].hal_handle); }
#endif
